...
export const getServiceProviderUsers = (params) => {
    return async (dispatch) => {
        dispatch({ type: Actions.GET_SERVICE_PROVIDER_USERS_LOADING });
        ...
        fialureOrData.result == ProcessResult.SUCCESS ?
            dispatch({ type: Actions.GET_SERVICE_PROVIDER_USERS_SUCCESS, payload: { items: fialureOrData.response.data } }) :
            dispatch({ type: Actions.GET_SERVICE_PROVIDER_USERS_FAILURE, payload: { failure: fialureOrData.failure } });
